  ...
  return (
    ...
    >
      <nav className="pad flex h-[64px] items-center justify-between">
        ...
        <ul className="hidden gap-0.5 lg:flex">
          {PINS.map((pin) => (
            <li key={pin.href}>
              <a
                href={pin.href}
                className="group flex items-center gap-2 rounded px-2.5 py-2 font-mono text-[11px] tracking-widest text-muted transition-colors hover:text-silk"
              >
        ...
        <a
          href="#contact"
          className="font-mono text-[11px] tracking-widest text-copper hover:text-signal lg:hidden"
        >
          MENU
        </a>
      </nav>
